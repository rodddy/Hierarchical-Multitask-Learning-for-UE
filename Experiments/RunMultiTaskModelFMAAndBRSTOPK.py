...
# 训练函数
def train(epoch, loss_weights=[ratio_fma, ratio_brs], TopK=3):
    ...
    if (epoch + 1) % save_interval == 0:
        torch.save({
            ...
            'loss': avg_loss,
                        ...
                        f'model_multitask_joint_epoch_{epoch + 1}_TopK_{TopK}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pth'))
...
def val(epoch):
    ...
    if (epoch + 1) % save_interval == 0:
        ...
        append_confusion_matrix_with_timestamp(metrics, epoch, save_dir='Models',file_name=f'fma_brs_log_TopK_{TopK}.txt')
        calculate_regression_metrics(all_fma_labels, all_fma_preds, model_name=f"multitask_fma_brs",save_dir='Models',file_name=f'fma_brs_log_TopK_{TopK}.txt')
...
if __name__ == "__main__":
    ...
    for epoch in range(Epoch):
        train(epoch, loss_weights=[0.5, 0.5],TopK=3)  # 可调整任务权重
        val(epoch)
